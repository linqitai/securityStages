...
export default {
  install(Vue) {
    ...
    Vue.prototype.createElement = function () {
      ...
      str += '<div class="crop_loading"><div class="crop_content"><img src="http://static.hpbanking.com/xg/uploads/files/2b957a350dabed4669939bbcbfe60747-32-32.gif"><div class="crop_text">图片上传中</div></div></div>'
    ...
    Vue.prototype.initFunction = function () {
      ...
      this.addEvent(this.clickBtn, 'click', function () {
        self.crop(self.options.compressRaTio)
      })
      // 取消事件
    ...
    Vue.prototype.crop = function (compressRaTio) {
      ...
      setTimeout(function () {
        ...
        image.src = imgData
        // 判断图片是否大于5000k,不大于直接上传，反之压缩
        if (imgData.length < (5000 * 1024)) {
          console.log(`图片大小${imgData.length},图片小于5M，直接上传`)
          // self.resultObj.src = imgData
          // 图片上传
          self.postImg(imgData)
        } else {
          console.log(`图片大小${(imgData.length / (5000 * 1024))}M,图片大于5M，进行压缩`)
    ...
    Vue.prototype.destoried = function () {
      ...
      document.querySelector('.crop_success').style.display = 'none'
      // 移除事件
    ...
    Vue.prototype.postImg = function (data) {
      ...
      params.append('file', self.convertBase64UrlToBlob(data))
      params.append('type', self.type)
      params.append('applyId', self.applyId)
      console.log('params:')
      console.log(params)
      console.log(this.convertBase64UrlToBlob(data))
      console.log(self.type + ':' + self.applyId)
      let config = {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      } // 添加请求头
      // axios.post('/zyd/common/upload/idCard.json', params, config).then(res => {
      // }
      // 添加请求头
